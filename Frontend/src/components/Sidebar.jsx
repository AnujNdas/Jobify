  ...
  return (
    <div
      className={`min-h-screen p-4 transition-all duration-300 
        ...
        ${isDarkMode ? "bg-darkBg text-darkText" : "bg-gray-100 text-black"}`}
    >
      <button
      ...
      <h2 className={`text-xl font-bold mb-6 ${isCollapsed && "hidden"}`}>wwwork</h2>

      <ul className="space-y- mt-10">
        {menuItems.map((item) => (
          <li
            key={item.name}
      ...
      {/* Dark Mode Toggle */}
      <div className={`mt-20 p-4 rounded-lg ${isDarkMode ? "bg-darkCard" : "bg-gray-200"}`}>
        <button
